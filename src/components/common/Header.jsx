  ...
  return (
    <header className="sticky top-0 z-50 bg-slate-900/95 backdrop-blur-md border-b border-slate-800 text-slate-100 shadow-md">
      ...
      {(role === 'citizen' || role === 'worker') && (
        <div className="bg-gradient-to-r from-emerald-950 via-teal-900 to-slate-900 border-b border-emerald-500/20 px-4 py-1.5 flex items-center justify-between text-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-200">
              {t('truckNear')} <strong className="text-white">#KA-03-GH-1102</strong> {t('away')} <span className="text-emerald-300 font-bold">~3 mins</span>.
            </span>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={() => playChime('alert')}
              className="flex items-center gap-1 text-emerald-400 hover:text-emerald-300 bg-emerald-900/40 border border-emerald-700/50 px-2 py-0.5 rounded transition-all cursor-pointer"
      ...
      <div className="max-w-7xl mx-auto px-4 py-3 flex flex-wrap items-center justify-between gap-4">
        {/* Brand */}
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-slate-800/90 border border-emerald-500/30 p-1 flex items-center justify-center shadow-lg shadow-emerald-900/30 shrink-0">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 bg-slate-800/80 hover:bg-slate-700 border border-slate-700/80 rounded-lg px-2.5 py-1.5 text-xs text-slate-200 transition-all cursor-pointer"
          ...
          {/* Ward Selector (For Citizen, Supervisor, & Municipality) */}
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/40 text-emerald-300 px-2.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer shadow"
          ...
          {/* Google Maps Quick Config Button */}
          <button
            onClick={() => setShowMapKeyModal(true)}
            className="flex items-center gap-1.5 bg-slate-800/80 hover:bg-slate-700 border border-slate-700 text-cyan-300 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer shadow"
          ...
          <div className="relative">
            <button
              ...
              className="relative p-2 rounded-lg bg-slate-800/80 hover:bg-slate-700 border border-slate-700 text-slate-300 transition-all cursor-pointer"
              title={t('alerts')}
            >
            ...
            <div className="flex items-center gap-2 pl-2 border-l border-slate-800">
              <div className="flex items-center gap-2 bg-slate-800/90 border border-slate-700/80 rounded-xl px-2.5 py-1 text-xs">
                ...
                <div className="text-left hidden sm:block">
                  <span className="text-white font-bold block text-[11px] leading-tight max-w-[120px] truncate">
                    {currentUser.name}
                  </span>
              ...
              {/* Strict Logout Button (Returns to Login Gateway) */}
              <button
                onClick={logout}
                className="px-3 py-1.5 rounded-xl bg-rose-950/40 hover:bg-rose-900/60 border border-rose-800/60 text-rose-300 hover:text-white transition-all text-xs font-bold flex items-center gap-1.5 cursor-pointer shadow-sm"
          ...
          <button
            ...
            className="p-2 rounded-lg bg-slate-800/60 hover:bg-slate-700 border border-slate-700/60 text-slate-400 hover:text-slate-200 transition-all text-xs cursor-pointer"
